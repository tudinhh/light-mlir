...
struct MemRef {
    ...
    MemRef(std::vector<int64_t> s) : shape(std::move(s)) {
        int64_t totalSize = std::accumulate(shape.begin(), shape.end(), 1LL, std::multiplies<int64_t>());
...
public:
    void run(mlir::ModuleOp module) {
        ...
        for (mlir::Operation &op : mainFunc.getBlocks().front()) {
            ...
            throw std::runtime_error("Unsupported op: " + op.getName().getStringRef().str());
        }
    }
...
private:
    ...
    bool dispatch(mlir::Operation *op) {
        if (auto cOp = llvm::dyn_cast<mlir::arith::ConstantOp>(op)) { evalConstant(cOp); return true; }
        if (auto aOp = llvm::dyn_cast<mlir::arith::AddFOp>(op)) { evalAddF(aOp); return true; }
        if (auto alOp = llvm::dyn_cast<mlir::memref::AllocOp>(op)) { evalAlloc(alOp); return true; }
        if (auto dOp = llvm::dyn_cast<mlir::memref::DeallocOp>(op)) { evalDealloc(dOp); return true; }
        if (auto sOp = llvm::dyn_cast<mlir::memref::StoreOp>(op)) { evalStore(sOp); return true; }
        if (auto lOp = llvm::dyn_cast<mlir::memref::LoadOp>(op)) { evalLoad(lOp); return true; }
    ...
    void evalConstant(mlir::arith::ConstantOp op) {
        if (auto intAttr = llvm::dyn_cast<mlir::IntegerAttr>(op.getValue())) {
            ...
            if (op.getType().isIndex()) {
                stateMap[op.getResult()] = RuntimeValue{intAttr.getInt()}; // Stores as int64_t
            } else {
                stateMap[op.getResult()] = RuntimeValue{static_cast<int32_t>(intAttr.getInt())};
            }
        } else if (auto floatAttr = llvm::dyn_cast<mlir::FloatAttr>(op.getValue())) {
            stateMap[op.getResult()] = RuntimeValue{static_cast<float>(floatAttr.getValue().convertToFloat())};
        }
    }
    ...
    std::vector<int64_t> extractIndices(mlir::Operation::operand_range indicesOps) {
    ...
    void evalFor(mlir::scf::ForOp op) {
        ...
        std::cout << "Loop bound:" << lower <<" "<<upper<<" "<<step<<std::endl;
    }
...
int main(int argc, char **argv) {
    ...
    registry.insert<mlir::arith::ArithDialect, mlir::func::FuncDialect, mlir::memref::MemRefDialect, mlir::scf::SCFDialect>();
